		...
		try {
			...
			if (connection!=null) {
				if (SQLOperations.addItem(productbean, connection)) {
					...
					getServletContext().getRequestDispatcher("/additemstatus.jsp?status=true").forward(request, response);
				}
				else{
					getServletContext().getRequestDispatcher("/additemstatus.jsp?status=false").forward(request, response);
				}
			}else{
